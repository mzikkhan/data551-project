...
The **Global Sustainability Tracker** is an interactive visualization tool designed to help researchers, policy analysts, and NGOs explore the "World Sustainability Dataset" (2000–2018). Its primary objective is to illuminate the complex trade-offs between a nation's rapid economic expansion and its corresponding environmental and social footprints. By visualizing these three critical dimensions side-by-side, users can critically assess whether a country’s development is truly sustainable or if it comes at the significant cost of natural resource depletion and ecological damage.

### Single Entity Analysis & Interface Design
The dashboard's primary view, as sketched below, provides an exhaustive deep dive into the sustainability metrics of a single country, region, continent, or income group. The interface is meticulously organized into a cohesive grid layout featuring three main functional areas:
...
Each chart panel features internal dropdown menus, empowering users to toggle specific variables within that theme to test targeted hypotheses.

![Dashboard Sketch - Single Entity Analysis](screen_1.png)

### Comparative Analysis via Interactive Map
In addition to the deep-dive view, the app provides a robust comparative mode visualized through an interactive map interface. This feature allows users to select and compare two different countries or regions side-by-side. By leveraging a spatial visualization, users can identify regional clusters of sustainability trends and benchmark the performance of peer nations. This dual-view approach ensures that both micro-level details and macro-level global patterns are easily accessible to the user.
